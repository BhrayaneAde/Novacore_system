#!/usr/bin/env python3
"""Seeder corrigé basé sur complete_seed.py"""
...
tasks_data = [
    ...
    {"id": 6, "title": "Developper nouvelle fonctionnalite", "description": "API REST pour gestion clients", "assignedTo": 10, "assignedBy": 10, "departmentId": 6, "priority": "important", "status": "in_progress", "dueDate": "2025-02-01T17:00:00"},
]
...
leave_requests_data = [
    ...
    {"id": 4, "employeeId": 10, "type": "vacation", "startDate": "2025-03-01", "endDate": "2025-03-07", "days": 5, "status": "approved", "reason": "Vacances", "requestDate": "2025-01-15"},
]
...
evaluations_data = [
    ...
    {"id": 3, "employeeId": 10, "managerId": 10, "period": "2025-01", "globalScore": 78.2, "managerComments": "Bon travail technique.", "strengths": ["Competences techniques"], "improvements": ["Communication"], "nextObjectives": ["Formation soft skills"], "automaticMetrics": {"taskCompletion": 80, "deadlineRespect": 75}, "manualEvaluation": {"communication": 7, "teamwork": 8}},
]
...
def mark_seed_completed():
    with open(SEED_MARKER_FILE, 'w') as f:
        f.write(f"Seeding completed at: {datetime.now().isoformat()}")

def seed_data():
    if check_seed_status():
        logger.info("Le seeding a deja ete execute. Aucune action necessaire.")
        return
    
    logger.info("Demarrage du seeding de la base de donnees...")
    ...
    try:
        ...
        if existing_companies > 0:
            logger.info("Donnees deja presentes, seeding ignore")
            mark_seed_completed()
            return
            
        logger.info("Creation des tables si necessaire...")
        ...
        logger.info("Linking Users to Employees...")
        for user_data in users_data:
        ...
        notifications = [
            ...
            models.Notification(title="Evaluation terminee", message="Votre evaluation est prete", type="success", user_id=user_map[10].id, created_by_id=user_map[10].id),
        ]
        db.add_all(notifications)
        db.commit()
        
        # Marquer le seeding comme terminé
        mark_seed_completed()
...
if __name__ == "__main__":
    logger.info("Lancement du script de seeding...")
    run_seeder()